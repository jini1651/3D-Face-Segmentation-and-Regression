...
def makeInputData(input_feature, SF):
    ...
    outofidx_6 = torch.zeros((B, 1, C)).to(device) - 1
    ...
    # print(features.shape, near_features.shape)
...
class UpSampling(nn.Module):
    ...
    def forward(self, data):
        ...
        if out_n != N+cnt:
            print("in_n : ", in_n, " out_n : ", out_n, " cnt : ", N+cnt)
...
class LastLayer(nn.Module):
    ...
    def forward(self, original, projected):
        ...
        new_feature = torch.cat((original, feature), 2)      #BxNx(3+16+C0)
