...
from data_utils.myModelNetDataLoader import ModelNetDataLoader
...
def parse_args():
    ...
    parser.add_argument('--model', default='SFCNN', help='model name [default: pointnet_cls]')
...
def main(args):
    ...
    shutil.copy('models/SFCNN.py', str(exp_dir))
    ...
    classifier = model.get_model(num_class, SF_list=SFs, normal_channel=False).to(device)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            points = np.reshape(points[:, :, 0:3], (-1, args.num_point, 3))                 #chuga
            points = projection_utils.PCtoSF(points, SF3)                               #chuga
            points = np.reshape(points, (-1, 642, 1))
            ...
            pred, l3_feature = classifier(points)
